A null check (e.g., `x == null`, `x != null`, `Objects.nonNull(x)`, or `Objects::nonNull`) is redundant if it is performed on a variable or method call that is known to be non-null within a `@NullMarked` scope.
